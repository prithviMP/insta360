...
public class MoreSettingActivity extends BaseObserveCameraActivity {
    ...
    private Spinner mSpRawType;
    private RawType mCurrentRawType;

    private void resetPhotoFileType() {
        mCurrentRawType = RawType.nativeValueOf(InstaCameraManager.getInstance().getRawType(getCurrentFuncMode()));
        mSpRawType = findViewById(R.id.sp_ft);
        ArrayAdapter<RawType> adapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item);
        adapter.addAll(InstaCameraManager.getInstance().getSupportRawTypeList(getCurrentFuncMode()));
        mSpRawType.setAdapter(adapter);
        mSpRawType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (mCurrentRawType != adapter.getItem(position)) {
                    mCurrentRawType = adapter.getItem(position);
                }
            }
        ...
        if (mCurrentRawType != null) {
            mSpRawType.setSelection(adapter.getPosition(mCurrentRawType));
        }

        findViewById(R.id.btn_set_ft).setOnClickListener(v -> {
            if (mCurrentRawType != null) {
                InstaCameraManager.getInstance().setRawType(getCurrentFuncMode(), mCurrentRawType);
            }
        });
